...
export function accountV1SessionGetResponseToJSON(
  accountV1SessionGetResponse: AccountV1SessionGetResponse,
): string {
  return JSON.stringify(
    AccountV1SessionGetResponse$outboundSchema.parse(
      accountV1SessionGetResponse,
    ),
  );
}

export function accountV1SessionGetResponseFromJSON(
  jsonString: string,
): SafeParseResult<AccountV1SessionGetResponse, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => AccountV1SessionGetResponse$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'AccountV1SessionGetResponse' from JSON`,
  );
}
