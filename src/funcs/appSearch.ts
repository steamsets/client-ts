...
> {
  const parsed = safeParse(
    request,
    (value) => operations.AppSearchRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.AppSearchRequestBody, {
    explode: true,
  });

  const path = pathToFunc("/v1/app.search")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.AppSearchResponse$inboundSchema, {
      key: "AppSearchResponseBody",
    }),
