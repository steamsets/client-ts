...
> {
  const parsed = safeParse(
    request,
    (value) => operations.TrackEventRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.AnalyticsTrackEventRequestBody, {
    explode: true,
  });

  const path = pathToFunc("/v1/analytics.trackEvent")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.TrackEventResponse$inboundSchema, {
      key: "AnalyticsTrackEventResponseBody",
    }),
