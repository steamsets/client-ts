...
export function groupSearch(
  ...
  Result<
    operations.GroupSearchResponse,
    | errors.ErrorModel
    | SteamSetsError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GroupSearchRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.GroupSearchRequestBody, {
    explode: true,
  });

  const path = pathToFunc("/v1/group.search")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GroupSearchResponse$inboundSchema, {
      key: "GroupSearchResponseBody",
    }),
