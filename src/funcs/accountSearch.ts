...
export function accountSearch(
  ...
  Result<
    operations.AccountSearchAccountsResponse,
    | errors.ErrorModel
    | SteamSetsError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.AccountSearchAccountsRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.AccountSearchAccountsRequestBody, {
    explode: true,
  });

  const path = pathToFunc("/v1/account.searchAccounts")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.AccountSearchAccountsResponse$inboundSchema, {
      key: "AccountSearchAccountsResponseBody",
    }),
