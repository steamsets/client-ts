...
async function run() {
  const result = await steamSets.group.leaderboardV1GetGroup({
    end: 10,
...
import { groupLeaderboardV1GetGroup } from "@steamsets/client-ts/funcs/groupLeaderboardV1GetGroup.js";
...
async function run() {
  const res = await groupLeaderboardV1GetGroup(steamSets, {
    end: 10,
    goToLeaderboardRank: 1,
    leaderboard: "most_members",
    start: 1,
