...
async function run() {
  const result = await steamSets.badge.appV1ListBadges({
    appId: 701216,
  });
...
import { badgeAppV1ListBadges } from "@steamsets/client-ts/funcs/badgeAppV1ListBadges.js";
...
async function run() {
  const res = await badgeAppV1ListBadges(steamSets, {
    appId: 701216,
  });
